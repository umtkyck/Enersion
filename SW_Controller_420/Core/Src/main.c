...
static uint32_t analogUpdateTimer = 0;
...
void HandleRead420mA(const RS485_Packet_t* packet)
{
    uint8_t analogData[NUM_420MA_CHANNELS * 6];  // 26 channels * 6 bytes each
    AnalogInput_GetAll420mA(analogData, sizeof(analogData));
    
    RS485_SendResponse(packet->srcAddr, CMD_ANALOG_420_RESPONSE, 
                      analogData, sizeof(analogData));
    
    DEBUG_DEBUG("4-20mA data sent (26 channels)");
}
...
void HandleReadVoltage(const RS485_Packet_t* packet)
{
    uint8_t analogData[NUM_VOLTAGE_CHANNELS * 6];  // 6 channels * 6 bytes each
    AnalogInput_GetAllVoltage(analogData, sizeof(analogData));
    
    RS485_SendResponse(packet->srcAddr, CMD_ANALOG_VOLTAGE_RESPONSE, 
                      analogData, sizeof(analogData));
    
    DEBUG_DEBUG("0-10V data sent (6 channels)");
}
...
void HandleReadNTC(const RS485_Packet_t* packet)
{
    uint8_t ntcData[NUM_NTC_CHANNELS * 6];  // 4 channels * 6 bytes each
    AnalogInput_GetAllNTC(ntcData, sizeof(ntcData));
    
    RS485_SendResponse(packet->srcAddr, CMD_NTC_RESPONSE, 
                      ntcData, sizeof(ntcData));
    
    DEBUG_DEBUG("NTC temperature data sent (4 channels)");
}
...
void HandleReadAllAnalog(const RS485_Packet_t* packet)
{
    uint8_t allData[TOTAL_ANALOG_CHANNELS * 6];  // 36 channels * 6 bytes each
    AnalogInput_GetAllData(allData, sizeof(allData));
    
    RS485_SendResponse(packet->srcAddr, CMD_ALL_ANALOG_RESPONSE, 
                      allData, sizeof(allData));
    
    DEBUG_DEBUG("All analog data sent (36 channels)");
}
