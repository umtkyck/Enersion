/**
 * @file enersion_crc.c
 * @brief Enersion CRC16 Implementation - MISRA C:2012 Compliant
 * @version 1.0.0
 * 
...
/* ============================================================================
 * CRC Lookup Table - MISRA C:2012 Rule 8.9
 * ============================================================================ */

/**
 * Pre-computed CRC-16/MODBUS lookup table
 * MISRA C:2012 Rule 8.4: Compatible declarations
 */
static const uint16_t crc_table[256U] = {
...
{
    return enersion_crc16_update(ENERSION_CRC_INIT, data, length);
}
...
uint16_t enersion_crc16_update(uint16_t crc, const uint8_t *data, size_t length)
{
    ...
    /* MISRA C:2012 Rule 14.3: Null check */
    if (data != NULL) {
        for (size_t i = 0U; i < length; i++) {
            uint8_t index = (uint8_t)((result ^ data[i]) & 0xFFU);
            result = (uint16_t)((result >> 8U) ^ crc_table[index]);
        }
    }
...
bool enersion_crc16_verify(const uint8_t *data, size_t length)
{
    ...
    /* MISRA C:2012 Rule 14.3: Boundary check */
    if ((data != NULL) && (length >= 3U)) {
        /* Calculate CRC over data (excluding CRC bytes) */
        uint16_t calculated = enersion_crc16(data, length - 2U);
        
        /* Extract received CRC (little-endian) */
        uint16_t received = (uint16_t)data[length - 2U] |
...
void enersion_crc16_append(uint8_t *data, size_t length)
{
    /* MISRA C:2012 Rule 14.3: Null check */
    if (data != NULL) {
        uint16_t crc = enersion_crc16(data, length);
        
        /* Append CRC in little-endian format */
        data[length] = (uint8_t)(crc & 0xFFU);
